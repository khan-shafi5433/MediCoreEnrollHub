      ...
      <div className="p-6 rounded-3xl" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={revenueData}>
            ...
            <Tooltip contentStyle={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "0.75rem", fontFamily: "Inter", fontSize: "0.8rem" }} formatter={(v: number) => [`₹${(v / 100000).toFixed(2)}L`, "Revenue"]} />
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
      ...
      <div className="flex flex-col gap-4">
        {recentPayments.map((payment, i) => {
          const sc = paymentStatusConfig[payment.status as keyof typeof paymentStatusConfig];
